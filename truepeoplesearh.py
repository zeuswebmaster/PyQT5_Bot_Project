...
class TruepeopleSearch(QThread):
    ...
    def _intialize(self):
        ...
        header = ["Name", "Address", "Phone1", "phone2", "phone3", "phone4", "phone5", "phone6", "phone7", "phone8", "phone9", "phone10", "email1", "email2", "email3", "email4", "email5", "email6", "email7", "email8", "email9", "email10"]
        ...
        for info in infos:
            ...
            if len(common) == 2:
                ...
                try:
                    ...
                    for page in range(1, page_counts + 1):
                        if page != 1:
                            driver.get((owner_url + "&page={}").format(page))
                            time.sleep(2)
                    
                        ownerXpaths = driver2.find_elements_by_xpath("//div[contains(@class, 'card-summary')]//div[@class='h4']")
                        
                        viewButtons = driver2.find_elements_by_xpath("//div[contains(@class, 'card-summary')]//div[contains(@class, 'align-self-center')]/a")
                        ...
                        for ownerXpath, viewButton in zip(ownerXpaths, viewButtons):
                            ...
                            if flag == 2:
                                second_url = viewButton.get_attribute('href')
                                driver1.get(second_url)
                                parse_owner(driver1.page_source, driver1)
                                break
                except:
